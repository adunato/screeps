...
Creep.prototype.containerWithMostEnergy = function (energySources) {
    ...
    for(var key in energySources){
        if(energySources[key] instanceof StructureContainer) {
            if (_.sum(energySources[key].store) > energy) {
                energy = _.sum(energySources[key].store);
...
Creep.prototype.withdrawEnergyFromSourceContainer = function () {
    var room = Game.rooms[this.getSquad().getSquadRoomName()];
    var containers = cache.findSourceObjects(room);
    return this.withdrawEnergyFromSources(containers);
};
